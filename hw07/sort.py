...
while True:
    try:
        nums = input_nums() # User enters a list of numbers.
        break

    #Print an error message if the user enters anything other than a number.
    except ValueError:
...
for current_position in range(max_position):
    ...
    current_min = current_position # The current minimum begins as the current position.
    ...
    for test_position in range(current_position + 1 , max_position):
        ...
        # This swaps the current minimum for the current position after each test of each position in the list. 
